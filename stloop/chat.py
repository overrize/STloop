...
PROMPT_DATASHEET = """
请提供芯片手册路径（PDF），或输入 skip 使用默认 STM32F411RE：
> """
...
def _build_llm_prompt(
    ...
    if datasheet_path and datasheet_path.exists():
        datasheet_text = _extract_pdf_text(datasheet_path)
        if datasheet_text:
            parts.append(f"\n\n【芯片手册内容参考】\n{datasheet_text}")
        else:
            parts.append(f"\n\n用户提供了芯片手册: {datasheet_path}，目标芯片请据此确认。")
    return "".join(parts)
...
def run_interactive(client: STLoopClient, output_dir: Optional[Path] = None) -> int:
    ...
    if datasheet_input and datasheet_input.lower() not in ("skip", "s", ""):
        p = Path(datasheet_input)
        if p.exists():
            datasheet_path = p.resolve()
            print(f"  已使用芯片手册: {datasheet_path}")
        else:
            print(f"  文件不存在，已跳过: {datasheet_input}")

    full_prompt = _build_llm_prompt(requirement, schematic_path, datasheet_path)
