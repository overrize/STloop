...
def _setup_instructions(console: Console) -> bool:
    ...
    如果用户完成配置返回 True，否则返回 False
    """
    from rich.prompt import Prompt, Confirm

    console.print(
        Panel(
            "[cyan]Welcome to STLoop![/cyan]\n\n"
            "To generate code with AI, you need to configure an LLM API.",
            title="[yellow]API Configuration Required[/yellow]",
            border_style="yellow",
        )
    )
    ...
    console.print("  1. Kimi (Moonshot) - [dim]Recommended for Chinese users[/dim]")
    console.print("  2. OpenAI")
    console.print("  3. Other (custom)")

    choice = Prompt.ask("[cyan]Select option[/cyan]", choices=["1", "2", "3"], default="1")
    ...
    console.print(f"\n[bold]Configure {provider_name}:[/bold]")

    # 输入 API Key
    api_key = Prompt.ask(f"[cyan]Enter API Key[/cyan] [dim]({key_hint})[/dim]", password=True)
    ...
    if choice == "3":
        base_url = Prompt.ask(
            "[cyan]Enter API Base URL[/cyan] [dim](e.g., https://api.example.com/v1)[/dim]"
        )
    else:
        base_url = default_base

    # 输入 Model (可选)
    model = Prompt.ask("[cyan]Enter Model[/cyan]", default=default_model)
    ...
    if Confirm.ask("\n[cyan]Save to .env file?[/cyan]", default=True):
        try:
            # 保存到 .env 文件
            env_path = Path.cwd() / ".env"
            env_content = f"""# STLoop LLM Configuration
# Generated by stloop configuration wizard
...
"""
            ...
            console.print(f"[green][OK] Configuration saved to {env_path}[/green]")
            console.print("[dim]You can edit this file anytime to change settings.[/dim]")
            return True

        except Exception as e:
            console.print(f"[red][X] Failed to save configuration: {e}[/red]")
            # 尝试仅设置环境变量
            import os
